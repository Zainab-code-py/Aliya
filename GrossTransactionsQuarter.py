...
result = pd.DataFrame(columns=['Brand', 'TY Transactions', 'LY Transactions', 'vs LY'])
...
# Calculate the total transaction count for each brand for TY and LY
for brand in all_brands:
    total_transactions_ty = 0
    total_transactions_ly = 0

    # Calculate TY transactions
    if brand in brands_ty:
        ...
        total_transactions_ty = brand_sales_ty['Order No.'].nunique()
        print(f"Total transactions TY for brand {brand}: {total_transactions_ty}")

    # Calculate LY transactions
    if brand in brands_ly:
        ...
        total_transactions_ly = brand_sales_ly['Order No.'].nunique()
        print(f"Total transactions LY for brand {brand}: {total_transactions_ly}")

    # Calculate vs LY
    if total_transactions_ly > 0:
        vs_ly = ((total_transactions_ty - total_transactions_ly) / total_transactions_ly) * 100
    else:
        vs_ly = float('inf') if total_transactions_ty > 0 else 0

    print(f"Total Transactions for TY for brand {brand}: {total_transactions_ty}")
    print(f"Total Transactions for LY for brand {brand}: {total_transactions_ly}")
...
result = result[['Brand', 'TY Transactions', 'LY Transactions', 'vs LY']]

# Print the result DataFrame
print("Result DataFrame:\n", result)

# Save the result to a CSV file and Excel file
result.to_csv('C:/Users/Aliya/Downloads/quarterly_gross_transactions.csv', index=False)
